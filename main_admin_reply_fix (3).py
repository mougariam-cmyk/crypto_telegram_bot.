...
from telegram.ext import (
    ...
    CallbackQueryHandler, ConversationHandler, filters, ContextTypes, ApplicationHandlerStop
)
...
async def group_message_guard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Normal group members/admins have effective_user. Anonymous admins may
    # appear as sender_chat instead, so do not discard those messages.
    if user and (user.id == context.bot.id or user.is_bot):
        return

    actor_name = (
        (user.first_name or user.username)
        if user
        else (message.sender_chat.title if message.sender_chat else "there")
    )
    ...
    if mentioned or replied_to_bot:
        member_name = actor_name or "there"
    ...
    if bad_links:
        await message.reply_text(
            f"⚠️ {actor_name or 'Please'}, this group is dedicated to {data.get('coin_name', 'the official project')}.\n"
    ...
    if await is_other_coin_mentioned(data, message_text):
        await message.reply_text(
            f"⚠️ {actor_name or 'Please'}, please keep this group focused on {data.get('coin_name', 'the official project')}.\n"
            "Promotion or discussion of other tokens is not allowed here."
        )


async def community_message_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Route community messages before the private ConversationHandler.

    A user who has an active private setup conversation can otherwise have a
    group message consumed by that ConversationHandler. Community messages
    must always reach the community guard, including admin messages.
    """
...
if __name__ == '__main__':
    ...
    # Community interaction handlers run BEFORE the private ConversationHandler.
    # This prevents an active private setup state from consuming an admin/member
    # message sent in a group. The router stops propagation after community
    # handling, so the message cannot be processed twice.
    application.add_handler(
        MessageHandler(filters.StatusUpdate.NEW_CHAT_MEMBERS, community_message_router),
        group=-1,
    )
    application.add_handler(
        MessageHandler(filters.TEXT & ~filters.COMMAND, community_message_router),
        group=-1,
    )
