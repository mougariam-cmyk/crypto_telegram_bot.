...
async def generate_group_ai_reply(data, member_name, member_message):
    """Generate a real Gemini reply for a direct community interaction.

    Community AI uses GEMINI_API_KEY_3..10 only. Keys are rotated so a
    temporarily unavailable/exhausted key does not block the reply path.
    """
    prompt = f"""
...
A real member named {member_name} directly mentioned or replied to you with:
---
{member_message}
---

Reply naturally to THIS exact message.

Rules:
- Answer the actual question or statement directly.
- If they ask about the project, use only the supplied project facts.
- Never invent listings, partnerships, transactions, announcements, prices,
  community statistics or future events.
- Never promise profits or guaranteed price movements.
...
- Do not use markdown asterisk stars.
- Do not start with a generic greeting unless the member actually greeted you.
...
"""
    ...
    if not clients:
        logging.warning("[GROUP AI] Gemini unavailable: no community API keys configured")
        return None

    logging.info("[GROUP AI] queued/started for %s: %s", member_name, member_message[:120])

    # Keep the retry path short. A failed/overloaded key should be replaced by
    # another community key without creating a long multi-attempt delay.
    max_attempts = min(3, len(clients))
    last_error = None

    for attempt, client in enumerate(clients[:max_attempts], start=1):
        key_number = attempt
        try:
            logging.info("[GROUP AI] Gemini key rotation slot %s started", key_number)
            ...
            if reply:
                logging.info("[GROUP AI] Gemini success on rotation slot %s", key_number)
                return reply

            last_error = "empty response"
            logging.warning("[GROUP AI] Gemini empty response on rotation slot %s", key_number)

        except asyncio.TimeoutError:
            last_error = "timeout"
            logging.warning("[GROUP AI] Gemini timeout on rotation slot %s", key_number)
        except Exception as exc:
            last_error = exc
            logging.warning("[GROUP AI] Gemini unavailable on rotation slot %s: %s", key_number, exc)

    logging.warning("[GROUP AI] Gemini unavailable after %s attempts: %s", max_attempts, last_error)
    return None
...
async def group_message_guard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mentioned or replied_to_bot:
        ...
        if reply:
            ...
            logging.info("[GROUP AI] fallback used: no reply sent because Gemini was unavailable")
        return
