...
def generate_free_post(data, slot_index: int):
    ...
    # Fifth slot: useful evergreen crypto/community information rather than another
    # pure hype post, keeping the Free feed varied without making a claim about a
    # live breaking event.
    items = get_content_pool_posts(category="crypto")
    post_text = random.choice(items).get("content") if items else None
    return post_text or choose_central_post(data) or get_fallback_message(data), "ad"
...
def generate_post(data, free_slot_index=None):
    ...
    # Simulated buy alerts are an optional promotional layer for paid plans.
...
async def public_menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    feature_titles = {
        ...
        "menu_raffle": "🎁 ALERT BUY SIMULATOR",
...
async def get_link_ratio(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Free does not need the simulated-buy configuration step.
    # Skipping it removes an unnecessary wait/click from the Free onboarding.
    if context.user_data.get('selected_plan') == 'free':
        context.user_data['enable_new_buy'] = False
        await query.answer("✅ Free setup completed", show_alert=False)
        return await finish_setup(update, context)
        ...
        try:
            ...
            for item in expired:
                ...
                upgrade_text = (
                    ...
                    "🆓 Free includes 4 AI hype posts/day.\n"
    ...
    while True:
        try:
            ...
            if current_data.get('selected_plan') == 'free':
                ...
                try:
                    markup = FREE_PLAN_AD_BUTTONS if ad_mode == 'ad' else None
                    await application.bot.send_message(
                        chat_id=channel,
                        text=post_text,
                        reply_markup=markup,
