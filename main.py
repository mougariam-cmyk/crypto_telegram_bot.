...
# COIN ANALYZER KEYS 13-15: dedicated AI Coin Analyzer only.
GEMINI_ANALYZER_API_KEYS = [
    os.getenv("GEMINI_API_KEY_13", "").strip(),
    os.getenv("GEMINI_API_KEY_14", "").strip(),
    os.getenv("GEMINI_API_KEY_15", "").strip(),
]
GEMINI_ANALYZER_API_KEYS = [k for k in GEMINI_ANALYZER_API_KEYS if k]
ANALYZER_GEMINI_KEY_INDEX = 0
logging.info("[COIN ANALYZER] Dedicated AI keys configured: %d/3", len(GEMINI_ANALYZER_API_KEYS))
...
# ==========================================
NETWORKS_FOR_POOL = ["BNB", "Base", "Solana", "Sui", "Arc", "Robinhood"]
...
async def ensure_daily_content_pool():
    """Ensure today's shared 33-post content pool exists.
...
"""
        ...
        CONTENT_POOL_CACHE["batch_date"] = str(today)
        logging.info("Central AI content pool refreshed: 33 posts (3 geo + 3 markets + 4 crypto + 5 questions + 18 network: 3 per supported network).")
        return True
...
ANALYZER_SUPPORTED_CHAINS = {"robinhood", "arc", "solana", "sui", "bsc", "base"}
...
# Normalize every known paid-plan representation before Analyzer access checks.
# The rest of MARSOF AI may store/display equivalent plan names in different forms.
ANALYZER_PLAN_ALIASES = {
    "free": "free",
    "1_month": "1_month",
    "starter": "1_month",
    "monthly": "1_month",
...
def analyzer_normalize_plan(value):
    raw = str(value or "").strip().lower().replace("-", "_").replace(" ", "_")
    return ANALYZER_PLAN_ALIASES.get(raw, raw or "free")
...
def analyzer_risk_engine(raw):
    """Deterministic risk signals and transparent metrics from verified analyzer data only."""
    risks = []
    positive = []
    metrics = {}
    liq = (raw.get("liquidity") or {}).get("usd")
    mc = (raw.get("token") or {}).get("market_cap")
    fdv = (raw.get("token") or {}).get("fdv")
    vol = (raw.get("market") or {}).get("volume_24h_usd")
    buys = int((raw.get("market") or {}).get("buys_24h") or 0)
    sells = int((raw.get("market") or {}).get("sells_24h") or 0)
    price_change = (raw.get("market") or {}).get("price_change_24h_pct")

    if liq is not None:
        metrics["Liquidity"] = f"${liq:,.2f}"
        if liq < 25000:
        ...
        risks.append({"category":"Liquidity Risk","level":"Unavailable","reason":"Reliable liquidity data was not collected."})

    if mc and liq is not None and liq > 0:
        ratio = liq / mc
        metrics["Liquidity / Market Cap"] = f"{ratio*100:.2f}%"
        if ratio < 0.01:
            ...
            positive.append({"category":"Market Structure","signal":f"Liquidity represents about {ratio*100:.2f}% of market cap."})

    if mc and fdv is not None and mc > 0:
        metrics["FDV / Market Cap"] = f"{fdv/mc:.2f}x"

    if vol is not None:
        metrics["24H Volume"] = f"${vol:,.2f}"
        if liq and liq > 0:
            vl = vol / liq
            metrics["Volume / Liquidity"] = f"{vl:.2f}x"
            if vl > 20:
                risks.append({"category":"Trading Activity Risk","level":"Moderate","reason":f"24h volume is about {vl:.1f}x reported liquidity."})
            else:
                positive.append({"category":"Trading Activity","signal":f"24h volume is about {vl:.2f}x reported liquidity."})
    else:
        risks.append({"category":"Trading Activity Risk","level":"Unavailable","reason":"Reliable 24h volume data was not collected."})

    metrics["Buys"] = f"{buys:,}"
    metrics["Sells"] = f"{sells:,}"
    if buys + sells > 0:
        metrics["Buy / Sell Ratio"] = f"{buys/sells:.2f}x" if sells else "N/A"
        positive.append({"category":"Trade Flow","signal":f"24h activity reports {buys:,} buys and {sells:,} sells."})
    if price_change is not None:
        metrics["24H Price Change"] = f"{price_change:.2f}%"

    for category, label in [
        ("Contract Security Risk", "contract security"),
        ("Authority Risk", "owner/authority controls"),
        ("Honeypot / Sell Risk", "honeypot/sell simulation"),
        ("Holder Concentration Risk", "holder distribution"),
        ("Tokenomics Risk", "supply/tokenomics data"),
        ("Tax Risk", "buy/sell tax data"),
    ]:
        risks.append({"category":category,"level":"Unavailable","reason":f"Reliable {label} was not collected by the current analyzer source."})

    return {"risk_signals": risks, "positive_signals": positive, "metrics": metrics}


def analyzer_investment_assessment(risks):
    """Deterministic classification based only on collected market-risk signals."""
    signals = risks.get("risk_signals", []) if isinstance(risks, dict) else []
    high = [x for x in signals if str(x.get("level", "")).lower() == "high"]
    moderate = [x for x in signals if str(x.get("level", "")).lower() == "moderate"]

    if high:
        return {
            "status": "negative",
            "label": "NOT VALID FOR INVESTMENT",
            "emoji": "🔴",
            "reason": "One or more high-risk market factors were detected in the available data.",
        }
    if len(moderate) >= 2:
        return {
            "status": "negative",
            "label": "NOT VALID FOR INVESTMENT",
            "emoji": "🔴",
            "reason": "Multiple moderate-risk market factors were detected in the available data.",
        }
    return {
        "status": "positive",
        "label": "VALID FOR INVESTMENT",
        "emoji": "🟢",
        "reason": "No high-risk market factor or multiple moderate-risk factors were detected in the available data.",
    }
...
def get_analyzer_gemini_clients_in_rotation():
    global ANALYZER_GEMINI_KEY_INDEX
    if not GEMINI_ANALYZER_API_KEYS:
        return []
    start = ANALYZER_GEMINI_KEY_INDEX % len(GEMINI_ANALYZER_API_KEYS)
    clients = []
    for offset in range(len(GEMINI_ANALYZER_API_KEYS)):
        idx = (start + offset) % len(GEMINI_ANALYZER_API_KEYS)
        clients.append(genai.Client(api_key=GEMINI_ANALYZER_API_KEYS[idx]))
    ANALYZER_GEMINI_KEY_INDEX = (start + 1) % len(GEMINI_ANALYZER_API_KEYS)
    return clients


async def analyzer_gemini_report(raw, risks):
    clients = get_analyzer_gemini_clients_in_rotation()
    if not clients:
        logging.error("[COIN ANALYZER] No dedicated AI keys configured (GEMINI_API_KEY_13-15).")
        return None
    import json
    prompt = f"""
You are the MARSOF AI interpretation layer for a crypto token analyzer.
DATA FIRST -> ANALYSIS SECOND -> AI INTERPRETATION THIRD.
Use ONLY the supplied verified JSON. Never invent missing holders, ownership, taxes,
contract security, honeypot results, partnerships, listings, social activity, catalysts,
or any other facts. Do not provide personalized financial advice, Buy/Sell instructions,
or price predictions. Clearly distinguish verified data from unavailable data.
...
Return concise English text with exactly these headings:
🤖 MARSOF AI INTERPRETATION
⚠️ KEY RISKS
🟢 POSITIVE SIGNALS
🔎 UNVERIFIED DATA
📋 CONCLUSION

The conclusion must explain what the supplied data indicates and what remains unverified.
Do not change the deterministic investment assessment.
"""
    last_error = None
    for client in clients:
        try:
            ...
            last_error = exc
            logging.error("[COIN ANALYZER] MARSOF AI key attempt failed: %s", exc)
            continue
    logging.error("[COIN ANALYZER] All dedicated MARSOF AI key attempts failed: %s", last_error)
    return None


def analyzer_user_plan(user_id, context):
    # Prefer the current conversation state when it already contains a paid plan,
    # then verify against the active-user database snapshot.
    cached = analyzer_normalize_plan(context.user_data.get("selected_plan", ""))
    try:
        for row in get_active_users():
            if str(row.get("user_id", "")) == str(user_id):
                db_plan = analyzer_normalize_plan(row.get("selected_plan", ""))
                if db_plan in ANALYZER_FULL_PLANS:
                    return db_plan
                # If DB has no usable plan, retain a valid paid plan from context.
                if cached in ANALYZER_FULL_PLANS:
                    return cached
                return db_plan
    except Exception as exc:
        logging.warning("[COIN ANALYZER] plan lookup failed: %s", exc)
    return cached
...
async def start_coin_analyzer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    access = "Full Risk + MARSOF AI analysis" if plan in ANALYZER_FULL_PLANS else "Raw verified data only"
    text = (
        "🔍 MARSOF AI — COIN ANALYZER\n\n"
        "Enter the token contract address.\n\n"
        "Supported networks: Robinhood Chain • Arc • Solana • Sui • BNB Chain • Base\n\n"
...
async def coin_analyzer_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not detected:
        await update.message.reply_text("❌ I couldn't recognize this as a supported token contract address.\n\nSend a valid Robinhood Chain / Arc / BNB Chain / Base EVM address, Sui object ID, or Solana token address.")
        return ANALYZER_INPUT
...
async def run_coin_analyzer(update, context, address, chain, pairs):
    try:
        ...
        if plan not in ANALYZER_FULL_PLANS:
            text = analyzer_raw_report(raw) + "\n\n💎 Full Risk Engine + MARSOF AI interpretation is available on eligible paid plans."
            keyboard = InlineKeyboardMarkup([
                [InlineKeyboardButton("💎 View Plans", callback_data="menu_plans")],
        ...
        assessment = analyzer_investment_assessment(risks)
        await (update.callback_query.message.reply_text("🤖 MARSOF AI is analyzing the verified data…") if update.callback_query else update.effective_message.reply_text("🤖 MARSOF AI is analyzing the verified data…"))
        ai = await analyzer_gemini_report(raw, risks)
        risk_lines = ["⚠️ RISK ENGINE"]
        for category, value in risks.get("metrics", {}).items():
            risk_lines.append(f"📊 {category}: {value}")
        risk_lines.append("")
        for x in risks.get("risk_signals", []):
            risk_lines.append(f"• {x['category']}: {x['level']} — {x['reason']}")
        report = analyzer_raw_report(raw) + "\n\n" + "\n".join(risk_lines)
        report += (
            "\n\n📋 INVESTMENT ASSESSMENT\n"
            f"{assessment['emoji']} {assessment['label']}\n"
            f"{assessment['reason']}\n"
            "⚠️ This is a deterministic assessment of the available market-risk data, not personalized financial advice.\n"
            "⚠️ Security, holder, authority, honeypot and tax data remain unverified where marked unavailable."
        )
        if ai:
            report += "\n\n" + ai
        else:
            report += "\n\n⚪ MARSOF AI interpretation is currently unavailable; the deterministic data/risk layer is still shown."
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton(f"{assessment['emoji']} {assessment['label']}", callback_data="analyzer_assessment_info")],
            [InlineKeyboardButton("🔍 Analyze Another Token", callback_data="menu_coin_analyzer")],
            [InlineKeyboardButton("📊 AI & Market Intelligence", callback_data="menu_intelligence")],
            [InlineKeyboardButton("🏠 Main Menu", callback_data="back_to_main")],
        ])
...
async def public_menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_network_analytics":
        text = (
            ...
            "🌐 Supported: BNB Chain • Base • Solana • Sui • Arc • Robinhood Chain\n\n"
            "📊 TVL\n💱 DEX Volume\n💰 Market Cap\n⚡ Network Activity\n🔥 Top Memecoins\n\n"
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            NETWORK: [
                ...
                CallbackQueryHandler(get_network_choice, pattern='^network_(BNB|Base|Solana|Sui|Arc|Robinhood|SKIP)$'),
                CallbackQueryHandler(back_to_edit_menu_handler, pattern='^back_to_edit_menu$'),
