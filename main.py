...
from database import (
    ...
    get_community_ad_usage,
    consume_community_ad,
    get_community_ad_limits,
    get_buy_monitor_state,
    save_buy_monitor_state
)
...
(
    ...
    LANGUAGE_SELECT, ONBOARDING_INTRO, AD_TITLE, AD_BODY, AD_MEDIA, AD_CONFIRM
) = range(23)
...
# ==========================================
NETWORKS_FOR_POOL = ["BNB", "Solana", "Sui", "Base", "Arc", "Robinhood"]
...
async def public_menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Community tools are connected to the Community Control Center.
    if data == "menu_partner_promotion":
        await query.answer("This feature has been removed.", show_alert=True)
        return await show_community_control_center(update, context)

    if data == "menu_advertisement":
        return await start_community_advertisement(update, context)

    if data == "menu_community_management":
        return await show_community_management(update, context)

    if data == "menu_analytics":
        return await show_community_analytics(update, context)
...
# ==========================================
# COMMUNITY ADVERTISEMENT ENGINE
# ==========================================
COMMUNITY_AD_LIMITS = {
...
async def start_community_advertisement(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    channel = context.user_data.get("channel")
    plan = str(context.user_data.get("selected_plan", "free")).lower()
    if not channel:
        await edit_flow_message(query, context, "❌ Select a community first.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="menu_community_center")]]))
        return EDIT_OPTIONS_MENU
    usage = await asyncio.to_thread(get_community_ad_usage, query.from_user.id, channel, plan)
    if not usage.get("allowed", False):
        await edit_flow_message(query, context, f"📢 ADVERTISEMENT\n\nYour plan has reached its advertisement limit.\n\n{usage.get('label', 'Limit reached')}\nUsed: {usage.get('used', 0)}\nRemaining: 0", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Community Control Center", callback_data="menu_community_center")]]))
        return EDIT_OPTIONS_MENU
    context.user_data["ad_draft"] = {"title": "", "body": "", "photo_file_id": None, "pin": False}
    await edit_flow_message(query, context, "📢 CREATE ADVERTISEMENT\n\n1️⃣ Send the advertisement title:", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ Cancel", callback_data="community_ad_cancel")]]))
    return AD_TITLE

async def community_ad_title(update: Update, context: ContextTypes.DEFAULT_TYPE):
    title = (update.message.text or "").strip()
    if not title:
        await update.message.reply_text("❌ Title cannot be empty.")
        return AD_TITLE
    context.user_data["ad_draft"]["title"] = title[:200]
    await update.message.reply_text("2️⃣ Send the advertisement text:")
    return AD_BODY

async def community_ad_body(update: Update, context: ContextTypes.DEFAULT_TYPE):
    body = (update.message.text or "").strip()
    if not body:
        await update.message.reply_text("❌ Advertisement text cannot be empty.")
        return AD_BODY
    context.user_data["ad_draft"]["body"] = body[:4000]
    await update.message.reply_text("3️⃣ Optional image: send a photo or press Skip.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⏭️ Skip image", callback_data="ad_skip_photo")],[InlineKeyboardButton("❌ Cancel", callback_data="community_ad_cancel")]]))
    return AD_MEDIA

async def community_ad_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message and update.message.photo:
        context.user_data["ad_draft"]["photo_file_id"] = update.message.photo[-1].file_id
        await update.message.reply_text("4️⃣ Pin this advertisement?", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📌 Yes, pin", callback_data="ad_pin_yes"), InlineKeyboardButton("No", callback_data="ad_pin_no")],[InlineKeyboardButton("❌ Cancel", callback_data="community_ad_cancel")]]))
        return AD_CONFIRM
    await update.message.reply_text("Send a photo or press Skip.")
    return AD_MEDIA

async def community_ad_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    if query.data == "community_ad_cancel":
        context.user_data.pop("ad_draft", None)
        return await show_community_control_center(update, context)
    if query.data == "ad_skip_photo":
        context.user_data["ad_draft"]["photo_file_id"] = None
        await query.edit_message_text("4️⃣ Pin this advertisement?", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📌 Yes, pin", callback_data="ad_pin_yes"), InlineKeyboardButton("No", callback_data="ad_pin_no")],[InlineKeyboardButton("❌ Cancel", callback_data="community_ad_cancel")]]))
        return AD_CONFIRM
    if query.data not in ("ad_pin_yes", "ad_pin_no"):
        return AD_CONFIRM
    ...
    try:
        if draft.get("photo_file_id"):
            msg = await context.bot.send_photo(chat_id=channel, photo=draft["photo_file_id"], caption=text)
        else:
            msg = await context.bot.send_message(chat_id=channel, text=text)
        if query.data == "ad_pin_yes":
            try:
                await context.bot.pin_chat_message(chat_id=channel, message_id=msg.message_id, disable_notification=False)
            except Exception as exc:
                logging.warning("[ADVERTISEMENT] Pin failed: %s", exc)
        if not await asyncio.to_thread(consume_community_ad, query.from_user.id, channel, plan):
            logging.warning("[ADVERTISEMENT] Could not record usage for %s", channel)
        context.user_data.pop("ad_draft", None)
        await query.edit_message_text("✅ Advertisement published successfully.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Community Control Center", callback_data="menu_community_center")]]))
    except Exception as exc:
        logging.error("[ADVERTISEMENT] Publication failed: %s", exc)
        await query.edit_message_text("❌ Advertisement could not be published. Check the bot's administrator/posting permissions.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="menu_community_center")]]))
    return EDIT_OPTIONS_MENU

async def show_community_analytics(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    channel = context.user_data.get("channel")
    text = (
        "📊 COMMUNITY ANALYTICS\n\n"
        f"Community: {channel or 'Not selected'}\n\n"
        "👥 Members: live Telegram count\n"
        "🟢 Joins: tracked\n"
        "🔴 Leaves: tracked\n"
        "⚠️ Violations: tracked\n"
        "📈 Activity: tracked\n"
        "📊 Trend: calculated from activity events\n"
        "🚫 Blocks: tracked"
    )
    await edit_flow_message(query, context, text, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔄 Refresh", callback_data="menu_analytics")],[InlineKeyboardButton("🔙 Community Control Center", callback_data="menu_community_center")]]))
    return EDIT_OPTIONS_MENU

async def show_community_management(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    text = (
        "🛡️ COMMUNITY MANAGEMENT\n\n"
        "🔗 Link protection — delete links that are not approved for the project and issue a violation.\n\n"
        "🖼️ Image policy — allow or block member images.\n\n"
        "🔤 Forbidden keywords — configure words that trigger a violation.\n\n"
        "📜 Contract protection — inspect member-posted contracts and delete contracts that do not belong to the configured token.\n\n"
        "⚠️ Warning system — repeated violations can escalate to a block.\n\n"
        "The configuration screen will be connected without changing the existing community AI reply path."
    )
    await edit_flow_message(query, context, text, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⚙️ Configure", callback_data="community_management_config")],[InlineKeyboardButton("🔙 Community Control Center", callback_data="menu_community_center")]]))
    return EDIT_OPTIONS_MENU

# ==========================================
# REAL BUY MONITOR — LIVE DATA, NO SNAPSHOT STORAGE
# ==========================================
GECKO_NETWORK_MAP = {"BNB": "bsc", "Solana": "solana", "Sui": "sui", "Base": "base"}
REAL_BUY_MONITOR_INTERVAL = 60
REAL_BUY_COOLDOWN_SECONDS = 15 * 60

def _gecko_get(path, params=None):
    response = requests.get("https://api.geckoterminal.com/api/v2" + path, params=params or {}, headers={"Accept": "application/json;version=20230203"}, timeout=15)
    response.raise_for_status()
    return response.json()

def fetch_real_token_buys(data):
    network = GECKO_NETWORK_MAP.get(str(data.get("network", "")).strip())
    contract = str(data.get("contract", "")).strip()
    if not network or not contract:
        return []
    try:
        pools = _gecko_get(f"/networks/{network}/tokens/{contract}/pools").get("data", [])
        if not pools:
            return []
        pool_address = pools[0].get("attributes", {}).get("address")
        if not pool_address:
            return []
        trades = _gecko_get(f"/networks/{network}/pools/{pool_address}/trades").get("data", [])
        buys = []
        for trade in trades:
            attrs = trade.get("attributes", {})
            side = str(attrs.get("trade_type", attrs.get("kind", ""))).lower()
            if side not in ("buy", "buying"):
                continue
            buys.append({"id": trade.get("id", ""), "amount_usd": float(attrs.get("volume_in_usd") or 0), "tx_hash": attrs.get("tx_hash", ""), "timestamp": attrs.get("block_timestamp") or attrs.get("block_created_at")})
        return buys
    except Exception as exc:
        logging.warning("[REAL BUY MONITOR] Provider error for %s: %s", contract, exc)
        return []

async def real_buy_monitor(application):
    while True:
        try:
            active_users = await asyncio.to_thread(get_active_users)
            for data in active_users:
                buys = await asyncio.to_thread(fetch_real_token_buys, data)
                if not buys:
                    continue
                state = await asyncio.to_thread(get_buy_monitor_state, data["user_id"], data["channel"])
                last_id = state.get("last_trade_id", "") if state else ""
                if not last_id:
                    # First observation establishes a cursor; never publish historical trades.
                    await asyncio.to_thread(save_buy_monitor_state, data["user_id"], data["channel"], buys[0]["id"], 0)
                    continue
                new_buys = []
                for buy in buys:
                    if buy["id"] == last_id:
                ...
                if not new_buys:
                    continue
                newest = new_buys[0]
                last_signal = float(state.get("last_signal_at", 0) or 0) if state else 0
                await asyncio.to_thread(save_buy_monitor_state, data["user_id"], data["channel"], newest["id"], last_signal)
                if time.time() - last_signal < REAL_BUY_COOLDOWN_SECONDS:
                    continue
                # Publish the largest real buy observed since the previous cursor.
                # No synthetic threshold is invented here; filtering/cooldown can be tuned later.
                buy = max(new_buys, key=lambda x: x.get("amount_usd", 0))
                msg = (f"🔥 REAL BUY DETECTED\n\n💎 {data.get('coin_name', 'Token')}\n"
                       f"💰 Buy size: ${buy['amount_usd']:,.2f}\n"
                       f"🧾 Transaction: {buy.get('tx_hash') or 'verified DEX trade'}\n\n"
                       "📡 Detected from live on-chain DEX market data.")
                try:
                    await application.bot.send_message(chat_id=data["channel"], text=msg)
                    await asyncio.to_thread(save_buy_monitor_state, data["user_id"], data["channel"], newest["id"], time.time())
                except Exception as exc:
                    logging.error("[REAL BUY MONITOR] Publish failed for %s: %s", data["channel"], exc)
        except asyncio.CancelledError:
            break
        except Exception as exc:
            logging.error("[REAL BUY MONITOR] Loop error: %s", exc)
        await asyncio.sleep(REAL_BUY_MONITOR_INTERVAL)
...
async def show_community_control_center(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Per-community command center. All community tools stay inside this center."""
...
async def get_x_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("🟡 BNB", callback_data='network_BNB'), InlineKeyboardButton("🟣 Solana", callback_data='network_Solana')],
        [InlineKeyboardButton("🔵 Sui", callback_data='network_Sui'), InlineKeyboardButton("🔷 Base", callback_data='network_Base')],
        [InlineKeyboardButton("⚫ Arc", callback_data='network_Arc'), InlineKeyboardButton("🔴 Robinhood", callback_data='network_Robinhood')],
        [InlineKeyboardButton("⏭️ SKIP — All Networks", callback_data='network_SKIP')],
        [InlineKeyboardButton("🔙 Back", callback_data='back_to_buy_link')]
    ])
    await send_flow_reply(update, context, "6️⃣ Select the blockchain network / ecosystem of the token:", reply_markup=InlineKeyboardMarkup(keyboard))
    return NETWORK
...
async def post_init(application):
    ...
    application.create_task(real_buy_monitor(application))
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            NETWORK: [
                ...
                CallbackQueryHandler(get_network_choice, pattern='^network_(BNB|Solana|Sui|Base|Arc|Robinhood|SKIP)$'),
                CallbackQueryHandler(back_to_edit_menu_handler, pattern='^back_to_edit_menu$'),
            ...
            EDIT_OPTIONS_MENU: [
                ...
                CallbackQueryHandler(start, pattern='^back_to_main$'),
                CallbackQueryHandler(community_ad_confirm, pattern='^(community_ad_cancel|ad_skip_photo|ad_pin_yes|ad_pin_no)$')
            ],
            AD_TITLE: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, community_ad_title),
                CallbackQueryHandler(community_ad_confirm, pattern='^community_ad_cancel$')
            ],
            AD_BODY: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, community_ad_body),
                CallbackQueryHandler(community_ad_confirm, pattern='^community_ad_cancel$')
            ],
            AD_MEDIA: [
                MessageHandler(filters.PHOTO, community_ad_media),
                CallbackQueryHandler(community_ad_confirm, pattern='^(ad_skip_photo|community_ad_cancel)$')
            ],
            AD_CONFIRM: [
                CallbackQueryHandler(community_ad_confirm, pattern='^(ad_pin_yes|ad_pin_no|community_ad_cancel)$')
            ],
            MSG_PER_HOUR: [
