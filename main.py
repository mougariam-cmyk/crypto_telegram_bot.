...
(
    ...
    LANGUAGE_SELECT, ONBOARDING_INTRO, CONTRACT_CONFIRM
) = range(20)
...
def _fetch_dexscreener_pairs(contract: str):
    url = f"https://api.dexscreener.com/latest/dex/tokens/{contract}"
    req = Request(url, headers={"User-Agent": "MARSOF-AI/1.0"})
    with urlopen(req, timeout=15) as response:
        if getattr(response, "status", 200) != 200:
            raise RuntimeError(f"DexScreener HTTP {getattr(response, 'status', 'unknown')}")
        return json.loads(response.read().decode("utf-8"))


async def detect_token_from_contract(contract: str):
    """Resolve token metadata from the contract. No AI is used for detection."""
    contract = (contract or "").strip()
    if not contract or len(contract) < 10 or len(contract) > 100 or any(ch.isspace() for ch in contract):
        return None, "Invalid contract address format."

    try:
        payload = await asyncio.to_thread(_fetch_dexscreener_pairs, contract)
    except Exception as exc:
        logging.warning("[ONBOARDING] Token lookup failed for %s: %s", contract, exc)
        return None, "I could not retrieve verified market data for this contract right now."

    pairs = payload.get("pairs") or []
    candidates = []
    for pair in pairs:
        network = DEXSCREENER_CHAIN_MAP.get(str(pair.get("chainId", "")).lower())
        if not network:
            continue
        base = pair.get("baseToken") or {}
        if str(base.get("address", "")).lower() != contract.lower():
            continue
        candidates.append((float((pair.get("liquidity") or {}).get("usd") or 0), pair, network))

    if not candidates:
        return None, "No supported network/pair was found for this contract. Supported networks: BNB, Solana, Sui, Base, Arc, Robinhood."

    _, pair, network = max(candidates, key=lambda x: x[0])
    base = pair.get("baseToken") or {}
    quote = pair.get("quoteToken") or {}
    market = pair.get("marketCap") or pair.get("fdv") or 0
    liquidity = (pair.get("liquidity") or {}).get("usd") or 0
    volume_24h = (pair.get("volume") or {}).get("h24") or 0
    price = pair.get("priceUsd") or ""
    buys = (pair.get("txns") or {}).get("h24", {}).get("buys") or 0
    sells = (pair.get("txns") or {}).get("h24", {}).get("sells") or 0

    result = {
        "contract": contract,
        "network": network,
        "coin_name": base.get("name") or "Unknown Token",
...
async def confirm_contract_detection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    if query.data == "contract_detection_retry":
        context.user_data.pop("token_snapshot", None)
        await edit_flow_message(
            query, context,
            "🪙 Send your Token Contract Address (CA).\n\nMARSOF AI will automatically detect the network and token data.",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="back_to_plans")]])
        )
        return CONTRACT
    if query.data != "contract_detection_confirm":
        return CONTRACT_CONFIRM

    snapshot = context.user_data.get("token_snapshot") or {}
    context.user_data["contract"] = snapshot.get("contract", "")
    context.user_data["coin_name"] = snapshot.get("coin_name", "Unknown Token")
    context.user_data["coin_symbol"] = snapshot.get("symbol", "UNKNOWN")
    context.user_data["network"] = snapshot.get("network", "")
    context.user_data["buy_link"] = snapshot.get("pair_url", "")
    context.user_data["x_link"] = ""
    context.user_data["coin_desc"] = ""
    context.user_data["receive_geopolitical_news"] = False
    context.user_data["receive_market_news"] = False
    context.user_data["market_monitoring_enabled"] = True
    context.user_data["market_monitoring_mode"] = "positive_signals_only"
    context.user_data["market_snapshot_storage"] = "runtime_only"
    context.user_data["onboarding_v2"] = True
    ...
    await edit_flow_message(
        query, context,
        f"✅ TOKEN CONFIRMED\n\n"
        f"🪙 {snapshot.get('coin_name', 'Unknown Token')} (${snapshot.get('symbol', 'UNKNOWN')})\n"
        f"🌐 Network: {snapshot.get('network', 'Unknown')}\n"
        f"📊 Market Cap: {_format_usd(snapshot.get('market_cap'))}\n"
        f"📈 24H Volume: {_format_usd(snapshot.get('volume_24h_usd'))}\n"
        f"🟢 Buys: {snapshot.get('buys_24h', 0):,}  |  🔴 Sells: {snapshot.get('sells_24h', 0):,}\n\n"
        "Next, connect the Telegram channel/group where MARSOF will publish market signals.\n\n"
        f"7️⃣ Send your Channel Username or Link.\nExample: @mychannel\n\n"
        f"🤖 After that, add {bot_username_display} as Administrator with Post Messages permission.",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="back_to_plans")]])
    )
    return CHANNEL
...
async def plan_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = (
        ...
        "🪙 Send your Token Contract Address (CA).\n\n"
        "MARSOF AI will automatically detect:\n"
        "• Network\n• Token name & symbol\n• Market data available from the source\n\n"
        "No manual token description, network selection, buy link, news category, or post type is required."
    )
...
async def get_contract(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get('is_editing'):
        # Preserve the existing edit workflow for now.
        keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data='back_to_edit_menu')]])
        await send_flow_reply(update, context, "Contract updated. Continue editing or save the configuration.", reply_markup=keyboard)
        return await show_edit_options(update, context)

    await send_flow_reply(update, context, "🔎 Detecting network and retrieving verified token data...", reply_markup=None)
    snapshot, error = await detect_token_from_contract(contract)
    if not snapshot:
        await send_flow_reply(
            update, context,
            f"❌ {error}\n\nPlease send a valid contract address from a supported network.",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data='back_to_plans')]])
        )
        return CONTRACT

    context.user_data['token_snapshot'] = snapshot
    await send_flow_reply(
        update, context,
        f"🔎 TOKEN DETECTED\n\n"
        f"🪙 {snapshot.get('coin_name', 'Unknown Token')} (${snapshot.get('symbol', 'UNKNOWN')})\n"
        f"🌐 Network: {snapshot.get('network', 'Unknown')}\n"
        f"💵 Price: {snapshot.get('price_usd') or 'N/A'}\n"
        f"📊 Market Cap: {_format_usd(snapshot.get('market_cap'))}\n"
        f"💧 Liquidity: {_format_usd(snapshot.get('liquidity_usd'))}\n"
        f"📈 24H Volume: {_format_usd(snapshot.get('volume_24h_usd'))}\n"
        f"🟢 Buys: {snapshot.get('buys_24h', 0):,}\n"
        f"🔴 Sells: {snapshot.get('sells_24h', 0):,}\n\n"
        "These values come from the market-data source. MARSOF AI does not invent them.",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("✅ Confirm Token", callback_data="contract_detection_confirm")],
            [InlineKeyboardButton("🔄 Try Another Contract", callback_data="contract_detection_retry")],
        ])
    )
    return CONTRACT_CONFIRM
...
async def verify_admin_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if status == 'administrator' and (can_post is True or can_post is None):
            if context.user_data.get('onboarding_v2'):
                context.user_data['msg_per_hour'] = 0
                context.user_data['link_ratio'] = 0
                context.user_data['enable_new_buy'] = False
                return await finish_market_onboarding(update, context)

            if context.user_data.get('selected_plan') == 'free':
                # Legacy/edit flow preserved.
                context.user_data['msg_per_hour'] = 0
                context.user_data['link_ratio'] = 0
                context.user_data['enable_new_buy'] = False
                return await finish_setup(update, context)
...
async def finish_market_onboarding(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    user_id = query.from_user.id
    username = query.from_user.username or "Unknown"

    # Save only the project configuration needed to run monitoring. Do NOT start
    # the legacy hourly AI publisher here; the market-signal engine is separate.
    context.user_data["market_monitoring_enabled"] = True
    context.user_data["market_monitoring_mode"] = "positive_signals_only"
    context.user_data["market_snapshot_storage"] = "runtime_only"
    context.user_data["msg_per_hour"] = 0
    ...
    token_name = context.user_data.get("coin_name", "Token")
    network = context.user_data.get("network", "Unknown")
    channel = context.user_data.get("channel", "your channel")
    plan = context.user_data.get("selected_plan", "free")

    text = (
        f"🎉 MARKET MONITORING ACTIVATED\n\n"
        f"🪙 Token: {token_name}\n"
        f"🌐 Network: {network}\n"
        f"📢 Channel: {channel}\n"
        f"💳 Plan: {plan.upper()}\n\n"
        "🤖 MARSOF will monitor real market activity and publish only significant positive signals.\n\n"
        "📊 Posts will use real market statistics such as buys, sells, volume and market-cap movement.\n"
        "🚫 No random political/economic/news posts.\n"
        "🚫 No invented statistics.\n"
        "💾 Continuous market snapshots are kept in runtime memory/cache, not stored as a historical database stream."
    )
    await edit_flow_message(query, context, text, reply_markup=InlineKeyboardMarkup([
        [InlineKeyboardButton("⚙️ Manage Community", callback_data='menu_edit_existing')],
        [InlineKeyboardButton("🏠 Main Menu", callback_data='back_to_main')]
    ]))
    context.user_data.clear()
    return ConversationHandler.END
...
if __name__ == '__main__':
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            CONTRACT_CONFIRM: [
                CallbackQueryHandler(confirm_contract_detection, pattern='^contract_detection_(confirm|retry)$'),
                CallbackQueryHandler(back_to_edit_menu_handler, pattern='^back_to_edit_menu$')
            ],
            BUY_LINK: [
