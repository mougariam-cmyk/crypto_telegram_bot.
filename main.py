...
SUPPORTED_NETWORKS = ["BNB", "Solana", "Sui", "Base", "Arc", "Robinhood"]
PLAN_DAILY_LIMITS = {"free": 5, "1_month": 8, "6_months": 30, "12_months": 80, "lifetime": 500}
...
# ==========================================
NETWORKS_FOR_POOL = SUPPORTED_NETWORKS
...
async def ensure_daily_content_pool():
    """Build a small non-political project/network content pool once per day."""
    global CONTENT_POOL_ATTEMPT_DATE, CONTENT_POOL_GENERATION_FAILED_TODAY
    async with CONTENT_POOL_LOCK:
        from datetime import date
        today = date.today()
        count, batch_date = await asyncio.to_thread(get_content_pool_stats)
        if count >= 18 and str(batch_date) == str(today):
        ...
        if not GEMINI_POST_API_KEYS:
            ...
            return False
        prompt = f"""
Create exactly 33 short English Telegram posts as valid JSON only.
Schema: [{{"category":"hype|update|interactive|network","network":"","content":"..."}}]
Create 5 hype posts, 5 project-update/development posts, 5 interactive community posts,
and exactly 3 network posts for EACH supported ecosystem.
Supported networks: {', '.join(SUPPORTED_NETWORKS)}.
IMPORTANT: no politics, no macroeconomics, no geopolitical news, no invented market statistics,
no fake announcements, no fake partnerships, no fake transactions and no price guarantees.
Network posts must discuss durable ecosystem/technology themes rather than pretend current events.
Keep posts short, natural, crypto-native and varied. Output JSON only.
"""
        try:
            client = get_next_post_gemini_client()
            items = _generate_pool_with_client(client, prompt) if client else []
            normalized=[]
            allowed={"hype","update","interactive","network"}
            for item in items:
                if not isinstance(item,dict) or not item.get("content"): continue
                cat=str(item.get("category","hype")).lower().strip()
                if cat not in allowed: continue
                net=normalize_network_name(str(item.get("network","")).strip()) if cat=="network" else ""
                if cat=="network" and net not in SUPPORTED_NETWORKS: continue
                normalized.append({"category":cat,"network":net,"slot":"any","content":str(item["content"]).strip()})
            # Require at least one of each family; otherwise retain the previous/local fallback.
            if not all(any(x["category"]==c for x in normalized) for c in ("hype","update","interactive")):
                CONTENT_POOL_GENERATION_FAILED_TODAY=True
                return False
            await asyncio.to_thread(replace_content_pool, normalized)
            CONTENT_POOL_CACHE["count"]=len(normalized)
            CONTENT_POOL_CACHE["batch_date"]=str(today)
            return True
        except Exception as e:
            logging.error("Content pool refresh failed: %s", e)
            CONTENT_POOL_GENERATION_FAILED_TODAY=True
            return False


def choose_central_post(data):
    """Choose only hype/update/interactive/network content."""
    import hashlib
    from datetime import date
    channel=str(data.get("channel","")); today=str(date.today())
    key=f"{channel}:{today}"
    seen=CONTENT_DELIVERY_STATE.setdefault(key,set())
    for k in list(CONTENT_DELIVERY_STATE):
        if not k.endswith(f":{today}"): CONTENT_DELIVERY_STATE.pop(k,None)
    eligible=[]
    for cat in ("hype","update","interactive"):
        eligible.extend(get_content_pool_posts(category=cat))
...
def normalize_network_name(value):
    ...
    aliases={"bnb":"BNB","bsc":"BNB","bnb chain":"BNB","binance smart chain":"BNB",
             "sol":"Solana","solana":"Solana","sui":"Sui","base":"Base","arc":"Arc",
             "robinhood":"Robinhood","robinhood chain":"Robinhood"}
    return aliases.get(value.lower(),value)


def _dex_chain_to_network(chain_id):
    return {"bsc":"BNB","solana":"Solana","sui":"Sui","base":"Base","arc":"Arc","robinhood":"Robinhood"}.get(str(chain_id).lower())


def _pick_socials(info):
    socials={}
    for item in (info or {}).get("socials") or []:
        typ=str(item.get("type","")).lower(); url=str(item.get("url","")).strip()
        if not url: continue
        if typ in ("twitter","x"): socials["x"]=url
        elif typ in ("telegram","discord","facebook","instagram","reddit"): socials[typ]=url
    for item in (info or {}).get("websites") or []:
        label=str(item.get("label", "website")).lower(); url=str(item.get("url","")).strip()
        if url and "website" not in socials: socials["website"]=url
    return socials


def fetch_token_snapshot(contract):
    """Fetch real token/pair data from DexScreener; never invent missing fields."""
    url=f"https://api.dexscreener.com/latest/dex/tokens/{quote(contract.strip())}"
    req=Request(url,headers={"User-Agent":"MARSOF-AI/1.0"})
    with urlopen(req,timeout=12) as response:
        payload=json.loads(response.read().decode("utf-8"))
    pairs=payload.get("pairs") or []
    supported=[p for p in pairs if _dex_chain_to_network(p.get("chainId"))]
    if not supported: return None,"No supported network pair was found for this contract."
    pair=max(supported,key=lambda p: float(((p.get("liquidity") or {}).get("usd") or 0)))
    base=pair.get("baseToken") or {}
    info=pair.get("info") or {}
    network=_dex_chain_to_network(pair.get("chainId"))
    socials=_pick_socials(info)
    website=socials.get("website","")
    x_link=socials.get("x","")
    snapshot={
        "coin_name":base.get("name") or "",
        "symbol":base.get("symbol") or "",
        "contract":contract.strip(),
        "network":network,
        "buy_link":pair.get("url") or "",
    ...
    if not snapshot["coin_name"] or not snapshot["symbol"]:
        return None,"Token metadata could not be read from the market-data source."
...
def build_market_change_report(previous,current):
    changes=[]
    def pct(a,b):
        try:
            a=float(a); b=float(b)
    ...
    for label,key,threshold in (("Price","price_usd",5),("Market cap","market_cap",10),("24h volume","volume_24h_usd",25),("Liquidity","liquidity_usd",15)):
        d=pct(previous.get(key),current.get(key))
        if d is not None and abs(d)>=threshold:
            arrow="📈" if d>0 else "📉"
            changes.append(f"{arrow} {label}: {d:+.1f}%")
    try:
        pb=int(previous.get("buys_24h") or 0); cb=int(current.get("buys_24h") or 0)
        ps=int(previous.get("sells_24h") or 0); cs=int(current.get("sells_24h") or 0)
        if abs(cb-pb)>=max(10,int(max(pb,1)*0.25)): changes.append(f"🟢 Buys (24h): {pb:,} → {cb:,}")
        if abs(cs-ps)>=max(10,int(max(ps,1)*0.25)): changes.append(f"🔴 Sells (24h): {ps:,} → {cs:,}")
    except Exception: pass
    if not changes: return None
    return (f"📊 {current.get('coin_name') or 'Token'} MARKET UPDATE\n\n"
            f"🪙 ${current.get('symbol','')} · {current.get('network','')}\n"
            + "\n".join(changes) +
            f"\n\n💧 Liquidity: ${_fmt_num(current.get('liquidity_usd'))}"
            f"\n📊 24h Volume: ${_fmt_num(current.get('volume_24h_usd'))}"
            f"\n🛒 {current.get('buy_link') or 'Buy link unavailable'}")


async def detect_token_from_contract(contract):
    try:
        return await asyncio.to_thread(fetch_token_snapshot, contract)
    except Exception as e:
        logging.warning("Token detection failed: %s", e)
        return None,"Unable to retrieve token data right now. Please try again."


async def market_monitor_loop(application,user_data):
    channel=user_data.get("channel"); user_id=user_data.get("user_id")
    if not channel or not user_id: return
    while True:
        try:
            data=await asyncio.to_thread(get_user_channel_data,user_id,channel)
            if not data or data.get("subscription_status")=="cancelled" or not data.get("contract"):
                return
            current,err=await detect_token_from_contract(data["contract"])
            if current:
                previous=data.get("last_market_snapshot")
                await asyncio.to_thread(update_market_snapshot,user_id,channel,current)
                if previous:
                    report=build_market_change_report(previous,current)
                    if report and await asyncio.to_thread(should_send_market_alert,user_id,channel):
                        try:
                            await application.bot.send_message(chat_id=channel,text=report,disable_web_page_preview=True)
                            await asyncio.to_thread(mark_market_alert_sent,user_id,channel)
                        except Exception as send_err:
                            logging.warning("Market report publish failed for %s: %s",channel,send_err)
        except asyncio.CancelledError: return
        except Exception as e: logging.error("Market monitor error for %s: %s",channel,e)
...
def generate_free_post(data, slot_index: int):
    ...
    if slot_index == 3:
        items = get_content_pool_posts(category="update")
        post_text = random.choice(items).get("content") if items else None
...
# ==========================================
# BUILD 03 — FAST AI COMMUNITY REPLY ENGINE
# ==========================================
GROUP_AI_QUEUE = None
GROUP_AI_WORKER_TASKS = []
GROUP_AI_DEDUPE = {}
GROUP_AI_INFLIGHT = set()
GROUP_AI_LOCK = asyncio.Lock()
GROUP_AI_COUNTER = 0
GROUP_AI_CONVERSATIONS = {}
GROUP_AI_MAX_QUEUE = 100
GROUP_AI_DEDUPE_SECONDS = 45
GROUP_AI_TIMEOUT_SECONDS = 9
GROUP_AI_WORKERS = 2

def _group_conversation_key(chat_id, user_id):
    return (str(chat_id), str(user_id))

def _group_message_fingerprint(chat_id, user_id, text):
    raw = f"{chat_id}|{user_id}|{(text or '').strip().lower()}"
    return hashlib.sha256(raw.encode('utf-8')).hexdigest()[:24]

def _remember_group_message(chat_id, user_id, text):
    key = _group_conversation_key(chat_id, user_id)
    history = GROUP_AI_CONVERSATIONS.setdefault(key, deque(maxlen=5))
    history.append((time.time(), (text or '').strip()[:500]))
    if len(GROUP_AI_CONVERSATIONS) > 500:
        oldest = min(GROUP_AI_CONVERSATIONS.items(), key=lambda item: item[1][0][0] if item[1] else 0)[0]
        GROUP_AI_CONVERSATIONS.pop(oldest, None)

def _get_group_context(chat_id, user_id):
    history = GROUP_AI_CONVERSATIONS.get(_group_conversation_key(chat_id, user_id), ())
    return [text for _, text in history if text]

def _queue_group_ai_job(priority, payload):
    global GROUP_AI_COUNTER
    if GROUP_AI_QUEUE is None or GROUP_AI_QUEUE.qsize() >= GROUP_AI_MAX_QUEUE:
        return False
    GROUP_AI_COUNTER += 1
    GROUP_AI_QUEUE.put_nowait((priority, GROUP_AI_COUNTER, payload))
    return True

async def generate_group_ai_reply(data, member_name, member_message, chat_id=None, user_id=None):
    context_lines = _get_group_context(chat_id, user_id) if chat_id is not None and user_id is not None else []
    previous = "\n".join(f"- {line}" for line in context_lines[-4:-1])
    prompt = f"""
...
Recent messages from this member, for context only:
{previous or '(none)'}

The member named {member_name} just wrote:
---
{member_message}
---

Reply naturally to THIS message.

Rules:
- Answer the actual question or statement directly.
- Use only supplied project facts. Never invent listings, partnerships, transactions, announcements, prices, community statistics or future events.
- Never promise profits or guaranteed price moves.
- If asked for a guaranteed future price, say briefly that it cannot be guaranteed.
- Be useful, concise and conversational.
- English only.
- Normally 1-4 short lines.
...
"""
    ...
    if not clients:
        return f"I'm here to help with {data.get('coin_name', 'the project')}."
    for attempt, client in enumerate(clients[:2], start=1):
        try:
            response = await asyncio.wait_for(asyncio.to_thread(client.models.generate_content, model="gemini-3.6-flash", contents=prompt), timeout=GROUP_AI_TIMEOUT_SECONDS)
            reply = (response.text or '').strip() if response else ''
            if reply:
                logging.info("Build03 group AI reply generated (attempt %s): %s", attempt, reply[:120])
                return reply
            logging.warning("Build03 group AI empty response on attempt %s", attempt)
        except asyncio.TimeoutError:
            logging.warning("Build03 group AI timeout on attempt %s", attempt)
        except Exception as exc:
            logging.warning("Build03 group AI failure on attempt %s: %s", attempt, exc)
    return f"I'm here to help with {data.get('coin_name', 'the project')}."

async def _group_ai_worker(worker_id):
    while True:
        try:
            _, _, job = await GROUP_AI_QUEUE.get()
            key = (job['chat_id'], job['user_id'])
            try:
                async with GROUP_AI_LOCK:
                    if key in GROUP_AI_INFLIGHT:
                        continue
                    GROUP_AI_INFLIGHT.add(key)
                reply = await generate_group_ai_reply(job['data'], job['member_name'], job['text'], chat_id=job['chat_id'], user_id=job['user_id'])
                try:
                    await job['bot'].send_message(chat_id=job['chat_id'], text=reply, reply_to_message_id=job['message_id'])
                except Exception as send_exc:
                    logging.warning("Build03 failed to send group AI reply: %s", send_exc)
            finally:
                async with GROUP_AI_LOCK:
                    GROUP_AI_INFLIGHT.discard(key)
                GROUP_AI_QUEUE.task_done()
        except asyncio.CancelledError:
            break
        except Exception as exc:
            logging.exception("Build03 group AI worker %s error: %s", worker_id, exc)

async def start_group_ai_workers(application):
    global GROUP_AI_QUEUE, GROUP_AI_WORKER_TASKS
    if GROUP_AI_QUEUE is not None:
        return
    GROUP_AI_QUEUE = asyncio.PriorityQueue()
    GROUP_AI_WORKER_TASKS = [application.create_task(_group_ai_worker(i + 1)) for i in range(GROUP_AI_WORKERS)]
    logging.info("Build03 AI Community Reply Engine started with %s workers", GROUP_AI_WORKERS)

async def queue_group_ai_reply(update, context, data, member_name, member_message, priority=0):
    message = update.effective_message
    chat = update.effective_chat
    user = update.effective_user
    if not message or not chat or not user:
        return False
    text = (member_message or '').strip() or 'Hello!'
    fingerprint = _group_message_fingerprint(chat.id, user.id, text)
    now = time.time()
    previous = GROUP_AI_DEDUPE.get(fingerprint)
    if previous and now - previous < GROUP_AI_DEDUPE_SECONDS:
        logging.info("Build03 duplicate group message skipped")
        return False
    GROUP_AI_DEDUPE[fingerprint] = now
    if len(GROUP_AI_DEDUPE) > 2000:
        cutoff = now - GROUP_AI_DEDUPE_SECONDS
        for k in [k for k, v in GROUP_AI_DEDUPE.items() if v < cutoff]:
            GROUP_AI_DEDUPE.pop(k, None)
    _remember_group_message(chat.id, user.id, text)
    job = {'bot': context.bot, 'chat_id': chat.id, 'user_id': user.id, 'message_id': message.message_id, 'member_name': member_name, 'text': text, 'data': data}
    queued = _queue_group_ai_job(priority, job)
    if queued:
        logging.info("Build03 queued AI reply for %s/%s", chat.id, user.id)
    return queued
...
async def group_message_guard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mentioned or replied_to_bot:
        ...
        queued = await queue_group_ai_reply(update, context, data, member_name, clean_message, priority=0)
        if not queued:
            await message.reply_text("⏳ The AI is busy for a moment. Please try again.")
        return
...
async def public_menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    feature_titles = {
        ...
        "menu_coin_analyzer": "🔍 AI COIN ANALYZER",
...
async def show_edit_options(update: Update, context: ContextTypes.DEFAULT_TYPE):
    data=context.user_data; channel=data.get('channel','Channel'); plan=data.get('selected_plan','free')
    text=(f"⚙️ PROJECT SETTINGS — {channel}\n\n💳 Plan: {plan.upper()}\n"
          f"🪙 Token: {data.get('coin_name','Not set')} (${data.get('token_symbol','')})\n"
          f"📜 Contract: {data.get('contract','Not set')}\n🌐 Network: {data.get('network','Not set')}\n"
          f"🛒 Buy / DEX: {data.get('buy_link','Not available')}\n"
          f"🌐 Website: {data.get('website_link','Not available')}\n𝕏 X: {data.get('x_link','Not available')}\n"
          f"📅 Posts/day: {data.get('posts_per_day',PLAN_DAILY_LIMITS.get(plan,5))}\n\n"
          "MARSOF AI keeps these public project details in the database and uses the contract as the source for live monitoring.")
    keyboard=[[InlineKeyboardButton("🔄 Refresh Token Data",callback_data='opt_refresh_token')],
              [InlineKeyboardButton("📅 Change Posts/Day",callback_data='opt_posts_day')],
              [InlineKeyboardButton("❌ Cancel / Stop Subscription",callback_data='opt_cancel_sub')],
              [InlineKeyboardButton("🛡️ Community Control Center",callback_data='community_control_center')],
              [InlineKeyboardButton("🏠 Back to Main Menu",callback_data='back_to_main')]]
    if update.callback_query: await edit_flow_message(update.callback_query,context,text,reply_markup=InlineKeyboardMarkup(keyboard))
    else: await send_flow_reply(update,context,text,reply_markup=InlineKeyboardMarkup(keyboard))
    return EDIT_OPTIONS_MENU
...
async def edit_options_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data=='opt_refresh_token':
        snap,err=await detect_token_from_contract(context.user_data.get('contract',''))
        if snap:
            context.user_data.update({'coin_name':snap['coin_name'],'token_symbol':snap['symbol'],'network':snap['network'],'buy_link':snap['buy_link'],'website_link':snap['website_link'],'x_link':snap['x_link'],'logo_url':snap['logo_url'],'social_links':json.dumps(snap.get('social_links',{}),ensure_ascii=False),'last_market_snapshot':snap})
            await asyncio.to_thread(save_user_data,query.from_user.id,query.from_user.username or 'Unknown',context.user_data)
            return await show_edit_options(update,context)
        await query.answer(err or 'Could not refresh token data',show_alert=True); return EDIT_OPTIONS_MENU
    if data=='opt_posts_day':
        limit=PLAN_DAILY_LIMITS.get(context.user_data.get('selected_plan','free'),5)
        if context.user_data.get('selected_plan')=='free':
            await query.answer('Free plan is fixed at 5 posts/day.',show_alert=True); return EDIT_OPTIONS_MENU
        await edit_flow_message(query,context,f'📅 Enter daily post count (1–{limit}):',reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('🔙 Back',callback_data='back_to_edit_menu')]]))
        return POSTS_PER_DAY
    if data=='opt_cancel_sub':
        channel=context.user_data.get('channel','')
        keyboard=[[InlineKeyboardButton('YES, Stop Auto-Promoter 🛑',callback_data='confirm_cancel_yes')],[InlineKeyboardButton('NO, Keep Publishing 🚀',callback_data='confirm_cancel_no')],[InlineKeyboardButton('🔙 Back',callback_data='back_to_edit_menu')]]
    ...
    if data=='back_to_main': return await start(update,context)
    return EDIT_OPTIONS_MENU
...
async def confirm_cancel_sub_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'confirm_cancel_yes':
        ...
        if channel in MARKET_MONITOR_TASKS:
            MARKET_MONITOR_TASKS[channel].cancel()
            del MARKET_MONITOR_TASKS[channel]
...
async def plan_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    limit=PLAN_DAILY_LIMITS.get(plan_key,5)
    msg=(f"✅ Plan Selected: {plan_key.upper()}\n\n"
         "🪙 Send ONLY your Token Contract Address (CA).\n\n"
         "MARSOF AI will automatically retrieve and store the available project data:\n"
         "• Token name & symbol\n• Network\n• DEX / Buy link\n• Official website\n• X / Twitter and other public socials\n• Logo\n• Live market data\n\n"
         "You do not need to type the token name, description, network, website or social links.")
    await edit_flow_message(query,context,msg,reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back",callback_data='back_to_plans')]]))
    return CONTRACT
...
async def get_contract(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await send_flow_reply(update,context,"🔎 Detecting the token and retrieving public project data…")
    snapshot,error=await detect_token_from_contract(contract)
    if not snapshot:
        await send_flow_reply(update,context,f"❌ {error}\n\nSend the contract again.",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back",callback_data='back_to_plans')]]))
        return CONTRACT
    context.user_data.update({
        'contract':snapshot['contract'],'coin_name':snapshot['coin_name'],'coin_desc':'',
        'token_symbol':snapshot['symbol'],'network':snapshot['network'],'buy_link':snapshot['buy_link'],
        'website_link':snapshot['website_link'],'x_link':snapshot['x_link'],'logo_url':snapshot['logo_url'],
        'social_links':json.dumps(snapshot.get('social_links',{}),ensure_ascii=False),
        'token_snapshot':snapshot,'last_market_snapshot':snapshot
    })
    limit=PLAN_DAILY_LIMITS.get(context.user_data.get('selected_plan','free'),5)
    text=(f"🪙 TOKEN DETECTED\n\n{name if False else snapshot['coin_name']} (${snapshot['symbol']})\n"
          f"🌐 {snapshot['network']}\n"
          f"📜 {snapshot['contract']}\n"
          f"🛒 {snapshot['buy_link'] or 'Not available'}\n"
          f"🌐 Website: {snapshot['website_link'] or 'Not available'}\n"
          f"𝕏 X: {snapshot['x_link'] or 'Not available'}\n\n"
          f"📊 Live market data captured for monitoring.\n\n"
          f"{'Free plan: 5 posts/day fixed.' if context.user_data.get('selected_plan')=='free' else f'Choose your daily post count (1–{limit}).'}")
    buttons=[]
    if context.user_data.get('selected_plan')=='free':
        buttons=[[InlineKeyboardButton("✅ Confirm Token & Continue",callback_data='contract_detection_confirm')]]
    else:
        buttons=[[InlineKeyboardButton("✅ Confirm Token",callback_data='contract_detection_confirm')],[InlineKeyboardButton("🔄 Try Another Contract",callback_data='contract_detection_retry')]]
    await send_flow_reply(update,context,text,reply_markup=InlineKeyboardMarkup(buttons))
    return CONTRACT_CONFIRM

async def contract_detection_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query=update.callback_query; await query.answer()
    if query.data=='contract_detection_retry':
        await edit_flow_message(query,context,"🪙 Send the Token Contract Address (CA):",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back",callback_data='back_to_plans')]]))
        return CONTRACT
    if query.data!='contract_detection_confirm': return CONTRACT_CONFIRM
    plan=context.user_data.get('selected_plan','free'); limit=PLAN_DAILY_LIMITS.get(plan,5)
    if plan=='free':
        context.user_data['posts_per_day']=5
        await edit_flow_message(query,context,"✅ Token confirmed.\n\nFree plan is fixed at 5 posts/day.\n\n📢 Now send your Telegram group/channel username or public link.",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back",callback_data='back_to_contract')]]))
        return CHANNEL
    await edit_flow_message(query,context,f"✅ Token confirmed.\n\nHow many posts per day? Enter a number from 1 to {limit}.",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back",callback_data='back_to_contract')]]))
    return POSTS_PER_DAY

async def get_posts_per_day(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try: count=int(update.message.text.strip())
    except ValueError: count=0
    limit=PLAN_DAILY_LIMITS.get(context.user_data.get('selected_plan','free'),5)
    if count<1 or count>limit:
        await send_flow_reply(update,context,f"❌ Enter a number between 1 and {limit}.")
        return POSTS_PER_DAY
    context.user_data['posts_per_day']=count
    context.user_data['msg_per_hour']=max(1,round(count/24))
    if context.user_data.get('is_editing'):
        await asyncio.to_thread(save_user_data, update.effective_user.id, update.effective_user.username or 'Unknown', context.user_data)
        return await show_edit_options(update, context)
    await send_flow_reply(update,context,"📢 Now send your Telegram group/channel username or public link.",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back",callback_data='back_to_contract')]]))
    return CHANNEL
...
async def get_enable_new_buy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    success_text = (
        ...
        f"💳 Plan: {plan.upper()}\n"
        f"⏱️ Frequency: {context.user_data.get('posts_per_day', 5)} posts/day\n"
    ...
    while True:
        try:
            ...
            if current_data.get('selected_plan') == 'free':
                ...
                posts_per_day = current_data.get('posts_per_day', 5) or 5
                sleep_seconds = 86400 / max(1, posts_per_day)
...
async def restart_all_active_tasks(application):
    ...
    for task in MARKET_MONITOR_TASKS.values():
        task.cancel()
    MARKET_MONITOR_TASKS.clear()
    ...
    for u_data in active_users:
        ...
        if channel not in MARKET_MONITOR_TASKS:
            MARKET_MONITOR_TASKS[channel] = asyncio.create_task(market_monitor_loop(application, u_data))

async def post_init(application):
    await start_group_ai_workers(application)
...
async def post_shutdown(application):
    for task in list(GROUP_AI_WORKER_TASKS):
        try:
            task.cancel()
        except Exception:
            pass
    GROUP_AI_WORKER_TASKS.clear()
...
if __name__ == '__main__':
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            POSTS_PER_DAY: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, get_posts_per_day),
                CallbackQueryHandler(back_to_edit_menu_handler, pattern='^back_to_edit_menu$'),
            ],
            CONTRACT_CONFIRM: [
                CallbackQueryHandler(contract_detection_handler, pattern='^contract_detection_')
            ],
            CHANNEL: [
