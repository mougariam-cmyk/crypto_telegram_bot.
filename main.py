...
# ==========================================
NETWORKS_FOR_POOL = ["BNB", "Solana", "Sui", "Arc", "Robinhood", "Base"]
...
async def ensure_daily_content_pool():
    """Build the shared project content pool once per day."""
    ...
    async with CONTENT_POOL_LOCK:
        ...
        if count >= 33 and str(batch_date) == str(today):
        ...
        CONTENT_POOL_GENERATION_FAILED_TODAY = False
        prompt = f"""
You are the official crypto-community content writer for MARSOF AI.
Create exactly 15 short English posts as valid JSON only.
Categories must be exactly: hype, update, interactive.
Return: [{{"category":"hype|update|interactive","slot":"any","content":"..."}}]
Create exactly 5 posts per category.

HYPE: energetic posts about the configured token/project identity, community momentum and attention.
UPDATE: project-development, ecosystem, roadmap, product or token-development style posts.
INTERACTIVE: engaging questions/challenges for the community.

Use the placeholder {{coin_name}} when referring to the project token.
Do NOT invent market statistics, buyers, sellers, volume, holders, wallets, prices, partnerships,
listings or on-chain activity. Do not create generic news posts. Do not give buy/sell advice.
Do not promise profits. Keep posts 3-6 short lines, crypto-native, with moderate emojis.
"""
        network_prompt = f"""
You are the network-content writer for MARSOF AI.
Create exactly 18 short English posts as valid JSON only.
Return: [{{"network":"NETWORK","category":"network","slot":"any","content":"..."}}]
Create exactly 3 posts for EACH network: {', '.join(NETWORKS_FOR_POOL)}.
Cover network technology, ecosystem development, builders, infrastructure and durable ecosystem themes.
Do NOT invent current statistics, prices, partnerships, listings or breaking events.
Do not give financial advice. Keep each post concise and engaging.
"""
        keys = [k for k in GEMINI_POST_API_KEYS[:2] if k]
        if not keys:
            CONTENT_POOL_GENERATION_FAILED_TODAY = True
            return False
        posts = _generate_pool_with_client(genai.Client(api_key=keys[0]), prompt)
        if not posts:
            CONTENT_POOL_GENERATION_FAILED_TODAY = True
            return False
        network_key = keys[1] if len(keys) > 1 else keys[0]
        network_posts = _generate_pool_with_client(genai.Client(api_key=network_key), network_prompt)
        if not network_posts:
            CONTENT_POOL_GENERATION_FAILED_TODAY = True
            return False
        normalized = []
        allowed = {"hype", "update", "interactive"}
        aliases = {"bnb":"BNB","bsc":"BNB","bnb chain":"BNB","binance smart chain":"BNB","sol":"Solana","solana":"Solana","sui":"Sui","arc":"Arc","robinhood":"Robinhood","robinhood chain":"Robinhood","base":"Base","base chain":"Base"}
        for item in posts:
            if isinstance(item, dict) and item.get("content") and str(item.get("category","")).lower() in allowed:
                normalized.append({"category":str(item["category"]).lower(),"network":"","slot":"any","content":str(item["content"]).strip()})
        ...
        counts={c:sum(1 for x in normalized if x["category"]==c) for c in ["hype","update","interactive","network"]}
        net_counts={n:sum(1 for x in normalized if x["category"]=="network" and x["network"]==n) for n in NETWORKS_FOR_POOL}
        complete=(counts["hype"]>=5 and counts["update"]>=5 and counts["interactive"]>=5 and all(net_counts[n]>=3 for n in NETWORKS_FOR_POOL))
        if not complete:
            CONTENT_POOL_GENERATION_FAILED_TODAY=True
            logging.warning("Central project content pool incomplete: counts=%s networks=%s",counts,net_counts)
            return False
        selected=[x for x in normalized if x["category"] in {"hype","update","interactive"}][:15]
        ...
        CONTENT_POOL_CACHE["batch_date"]=str(today)
        logging.info("Central AI content pool refreshed: %s project/network posts.", len(selected))
        return True


def choose_central_post(data):
    """Select shared project/network content; real market statistics are supplied by monitoring modules, not AI."""
    from datetime import date
    channel=str(data.get("channel","")); today=str(date.today())
    state_key=f"{channel}:{today}"; seen=CONTENT_DELIVERY_STATE.setdefault(state_key,set())
    for key in list(CONTENT_DELIVERY_STATE):
        if not key.endswith(f":{today}"): CONTENT_DELIVERY_STATE.pop(key,None)
    eligible=[]
    for category in ("hype","update","interactive"):
        eligible.extend(get_content_pool_posts(category=category))
    network=str(data.get("network","")).strip()
    if network:
        eligible.extend(get_content_pool_posts(category="network",network=network))
    else:
        for n in NETWORKS_FOR_POOL: eligible.extend(get_content_pool_posts(category="network",network=n))
    if not eligible: return None
    unseen=[x for x in eligible if x.get("id") not in seen]
    if not unseen: seen.clear(); unseen=eligible
    digest=int(hashlib.md5(channel.encode()).hexdigest()[:8],16)
    item=unseen[digest % len(unseen)]; seen.add(item.get("id"))
    return item["content"]


def generate_free_post(data, slot_index: int):
    """Free plan: hype, network/project content, plus one MARSOF ad. No generic news posts."""
    categories = {0:"hype", 1:"hype", 2:"network", 3:"update"}
    if slot_index in categories:
        category=categories[slot_index]
        if category=="network":
            items=get_content_pool_posts(category="network",network=str(data.get("network","")).strip()) if str(data.get("network","")).strip() else []
        else:
            items=get_content_pool_posts(category=category)
        if items:
            return random.choice(items).get("content") or choose_central_post(data) or get_fallback_message(data), ("ad" if slot_index in (2,3) else None)
        return choose_central_post(data) or get_fallback_message(data), ("ad" if slot_index in (2,3) else None)
    full_ad=(
...
async def public_menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_coin_analyzer":
        text = (
            "🔍 AI COIN ANALYZER\n\n"
            "🧩 Analyzer interface ready.\n\n"
            "The analysis engine will be connected to this button as a separate MARSOF AI module.\n"
            "No analyzer logic is executed in this bot build yet."
        )
        keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back to AI & Market Intelligence", callback_data="menu_intelligence")]])
        await edit_flow_message(query, context, text, reply_markup=keyboard)
        return MAIN_MENU
    if data == "menu_raffle":
...
async def show_edit_options(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    plan = str(data.get('selected_plan', 'free')).lower()
    posts_freq = 5 if plan == 'free' else data.get('posts_per_day', 8)
    text = (
        f"⚙️ CURRENT SETTINGS — {channel}\n\n"
        f"💳 Plan: {plan.upper()}\n"
        f"🪙 Token: {data.get('coin_name', 'Not set')} ({data.get('coin_symbol', '')})\n"
        f"📜 Contract: {data.get('contract', 'Not set')}\n"
        f"🌐 Network: {data.get('network', 'Not set')}\n"
        f"🛒 Buy Link: {data.get('buy_link', 'Not available')}\n"
        f"𝕏 X / Twitter: {data.get('x_link', 'Not available')}\n"
        f"🌐 Official Website: {data.get('website_link', 'Not available')}\n"
        f"📊 Posts per day: {posts_freq}\n\n"
        "All token/project metadata is automatically sourced from the contract and public market data when available."
    )
    keyboard = [
        [InlineKeyboardButton("📜 Edit Contract / Refresh Data", callback_data='opt_contract')],
        [InlineKeyboardButton("📊 Edit Posts Per Day", callback_data='opt_msg_hour')],
        [InlineKeyboardButton("❌ Cancel / Stop Subscription", callback_data='opt_cancel_sub')],
        [InlineKeyboardButton("🔙 Community Control Center", callback_data='menu_community_center')],
    ]
    await (update.callback_query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard)) if update.callback_query else send_flow_reply(update, context, text, reply_markup=InlineKeyboardMarkup(keyboard)))
    return EDIT_OPTIONS_MENU
...
async def edit_options_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == 'back_to_main':
        return await start(update, context)
    if data == 'opt_contract':
        keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data='back_to_edit_menu')]])
        await edit_flow_message(query, context, "📜 Send the token Contract Address (CA) to refresh all automatic project data:", reply_markup=keyboard)
        return CONTRACT
    if data == 'opt_msg_hour':
        plan = str(context.user_data.get('selected_plan', 'free')).lower()
        limits = {'free':5,'1_month':8,'6_months':30,'12_months':80,'annual':80,'lifetime':500}
        maximum = limits.get(plan,8)
        keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data='back_to_edit_menu')]])
        await edit_flow_message(query, context, f"📊 How many posts per day? (1–{maximum})", reply_markup=keyboard)
        return MSG_PER_HOUR
...
async def plan_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = (
        ...
        "🔗 Send only the token Contract Address (CA).\n\n"
        "MARSOF AI will automatically detect the network and retrieve the available token information, "
        "including token name, symbol, market links, official website and social links when publicly available."
    )
...
async def enrich_token_from_contract(contract: str):
    """Fetch public token metadata/market links from DexScreener without using Gemini."""
    result = {
        "contract": contract.strip(), "coin_name": "", "coin_symbol": "",
        "coin_desc": "", "network": "", "buy_link": "", "x_link": "",
        "website_link": "", "logo_url": "", "pair_url": "", "social_links": {},
    }
    url = f"https://api.dexscreener.com/latest/dex/tokens/{contract.strip()}"
    try:
        ...
        def liquidity_value(pair):
            try: return float((pair.get("liquidity") or {}).get("usd") or 0)
            except Exception: return 0.0

        pair = sorted(pairs, key=liquidity_value, reverse=True)[0]
        base = pair.get("baseToken") or {}
        result["coin_name"] = str(base.get("name") or "").strip()
        result["coin_symbol"] = str(base.get("symbol") or "").strip()
        result["network"] = normalize_network_name(str(pair.get("chainId") or ""))
        result["pair_url"] = str(pair.get("url") or "").strip()
        result["buy_link"] = result["pair_url"]

        info = pair.get("info") or {}
        result["logo_url"] = str(info.get("imageUrl") or "").strip()
        websites = info.get("websites") or []
        for site in websites:
            if isinstance(site, dict) and site.get("url"):
                result["website_link"] = str(site["url"]).strip()
                break
        socials = info.get("socials") or []
        for social in socials:
            if not isinstance(social, dict):
                continue
            stype = str(social.get("type") or "").lower()
            surl = str(social.get("url") or "").strip()
            if not surl:
                continue
            result["social_links"][stype or "other"] = surl
            if stype in {"twitter", "x"} and not result["x_link"]:
                result["x_link"] = surl

        # DexScreener may return several pairs; use the richest metadata available.
        if not result["website_link"] or not result["x_link"] or not result["logo_url"]:
            for candidate in pairs[:10]:
                cinfo = candidate.get("info") or {}
                if not result["logo_url"]:
                    result["logo_url"] = str(cinfo.get("imageUrl") or "").strip()
                if not result["website_link"]:
                    for site in cinfo.get("websites") or []:
                        if isinstance(site, dict) and site.get("url"):
                            result["website_link"] = str(site["url"]).strip(); break
                for social in cinfo.get("socials") or []:
                    if isinstance(social, dict) and social.get("url"):
                        stype = str(social.get("type") or "other").lower()
                        surl = str(social.get("url") or "").strip()
                        result["social_links"].setdefault(stype, surl)
                        if not result["x_link"] and stype in {"twitter", "x"}:
                            result["x_link"] = surl
        return result
    except Exception as exc:
        logging.warning("Automatic token metadata lookup failed for %s: %s", contract, exc)
        return result
...
async def get_contract(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not contract:
        await send_flow_reply(update, context, "❌ Please send a valid contract address.")
        return CONTRACT

    context.user_data['contract'] = contract
    await send_flow_reply(update, context, "🔎 Reading token data from the contract...\n\nPlease wait a moment.")
    token = await enrich_token_from_contract(contract)
    context.user_data.update(token)

    supported = {"BNB", "Solana", "Sui", "Arc", "Robinhood", "Base"}
    if not token.get("coin_name") or token.get("network") not in supported:
        await send_flow_reply(
            update, context,
            "⚠️ I could not retrieve enough public token data from this contract.\n\n"
            "Please check the contract address and send it again.",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data='back_to_edit_menu')]])
        )
        return CONTRACT
    ...
    context.user_data['coin_desc'] = context.user_data.get('coin_desc') or (
        f"{token.get('coin_name')} ({token.get('coin_symbol')}) on {token.get('network')}"
    )
    # Buy/X/website/network are populated automatically. User only chooses posting frequency next.
    plan = str(context.user_data.get('selected_plan', 'free')).lower()
    if plan == 'free':
        ...
        await send_flow_reply(update, context,
            f"✅ Token detected: {token.get('coin_name')} ({token.get('coin_symbol')})\n"
            f"🌐 Network: {token.get('network')}\n"
            f"🛒 Buy link: {'Found' if token.get('buy_link') else 'Not available'}\n"
            f"𝕏 X: {'Found' if token.get('x_link') else 'Not available'}\n"
            f"🌐 Website: {'Found' if token.get('website_link') else 'Not available'}\n\n"
            "🆓 Free plan: 5 posts/day. No additional configuration is required.",
            reply_markup=keyboard)
        return CHANNEL

    limits = {'1_month': 8, '6_months': 30, '12_months': 80, 'annual': 80, 'lifetime': 500}
    max_posts = limits.get(plan, 8)
    keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data='back_to_contract')]])
    await send_flow_reply(update, context,
        f"✅ Token detected: {token.get('coin_name')} ({token.get('coin_symbol')})\n"
        f"🌐 Network: {token.get('network')}\n\n"
        f"📊 How many posts per day do you want? (1–{max_posts})",
        reply_markup=keyboard)
    return MSG_PER_HOUR
...
async def get_msg_per_hour(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if count < 1 or count > maximum:
        await send_flow_reply(update, context, f"❌ Enter a number from 1 to {maximum} posts/day.")
        return MSG_PER_HOUR
    context.user_data['posts_per_day'] = count
    context.user_data['msg_per_hour'] = max(1, round(count / 24))
    if context.user_data.get('is_editing'):
        return await show_edit_options(update, context)
    return await continue_after_daily_count(update, context)


async def continue_after_daily_count(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back to Contract", callback_data='back_to_contract')]])
    await send_flow_reply(update, context,
        "📊 Daily posting frequency saved.\n\n"
        "Now send the Channel Username (e.g. @mychannel) or Channel Link (e.g. https://t.me/mychannel):",
        reply_markup=keyboard)
    return CHANNEL


async def get_buy_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Kept only for backward-compatible editing of legacy records. New onboarding never asks for it.
...
async def get_content_preferences(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Legacy compatibility only. Political/economic content preferences were removed.
    return await show_edit_options(update, context)

async def get_network_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Legacy callback compatibility. Network is now detected automatically from the contract.
    return await show_edit_options(update, context)


async def get_network(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Legacy text input compatibility for existing records only.
    context.user_data['network'] = normalize_network_name(update.message.text.strip())
    return await show_edit_options(update, context)

async def back_to_contract_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
...
async def verify_admin_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if status == 'administrator' and (can_post is True or can_post is None):
            plan = str(context.user_data.get('selected_plan', 'free')).lower()
            context.user_data['posts_per_day'] = 5 if plan == 'free' else context.user_data.get('posts_per_day', 8)
            context.user_data['msg_per_hour'] = max(1, round(context.user_data['posts_per_day'] / 24))
...
async def get_link_ratio(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data['link_ratio'] = 100
    return await finish_setup(update, context)
    ...
    while True:
        try:
            ...
            if current_data.get('selected_plan') == 'free':
                ...
                posts_per_day = current_data.get('posts_per_day') or current_data.get('msg_per_hour', 2) * 24
                sleep_seconds = (24 * 3600) / max(1, int(posts_per_day))
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            CHANNEL: [
                ...
                CallbackQueryHandler(back_to_edit_menu_handler, pattern='^back_to_edit_menu$'),
                CallbackQueryHandler(back_to_contract_handler, pattern='^back_to_contract$'),
